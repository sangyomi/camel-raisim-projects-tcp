    ...
    for (int leg = 0; leg < 4; leg++)
    {
        if (mGaitTable[leg] == 0)
        {
            if (bIsFirstRunSwing[leg] == true)
            {
                ...
                double yawRate = GAIT_PERIOD * 3/4* mBaseEulerVelocity[2];
...
//                mDesiredFootPosition[leg] = yawRateRot * (GetBaseRotationMat(mBaseDesiredQuaternion) * (mShoulderPosition[leg])) + mBasePosition + GAIT_PERIOD/ 2 * mBaseVelocity;
//                mDesiredFootPosition[leg] = GetBaseRotationMat(mBaseDesiredQuaternion) * (mShoulderPosition[leg]) + mBasePosition;
                ...
                mDesiredFootPosition[leg] = yawRateRot * GetBaseRotationMat(mBaseDesiredQuaternion) * (mShoulderPosition[leg]) + mBasePosition + GAIT_PERIOD*3/4 * mBaseVelocity
                    + (S - (S.dot(V))/(V.dot(V))*V);
//                mDesiredFootPosition[leg] = yawRateRot * (GetBaseRotationMat(mBaseDesiredQuaternion) * (mShoulderPosition[leg])) + mBasePosition + GAIT_PERIOD * 1/2 / 2.0 * mBaseVelocity + 0.5 * mBaseVelocity * mBaseEulerVelocity[2];
//                mDesiredFootPosition[leg] = yawRateRot * (GetBaseRotationMat(mBaseDesiredQuaternion) * (mShoulderPosition[leg])) + mBasePosition + GAIT_PERIOD/ 2 * mBaseVelocity;
...
void LowWholeBodyPDController::getJointVel(Vec3<double>& resultVel, Vec3<double> desiredPos, const int& leg, const bool stand)
{
    ...
    if (stand)
    {
        resultVel = jacobianInverse*rot*(mBaseDesiredVelocity) + jacobianInverse*base2footPosition.cross(mBaseDesiredEulerVelocity);
    }
    else
    {
        resultVel = jacobianInverse*rot*(mBaseDesiredVelocity - mSwingFootDesiredVelocity[leg]) + jacobianInverse*base2footPosition.cross(mBaseDesiredEulerVelocity);
    }
    for (int i = 0; i < 3; i++)
    {
