...
SwingLeg::SwingLeg()
//    : mTimeDuration(GAIT_PERIOD / 2)
: mTimeDuration(GAIT_PERIOD / 4)
{
...
void SwingLeg::SetFootTrajectory(const Vec3<double>& initPos, const Vec3<double>& desiredPos, const int& leg)
{
    ...
    {
//        px[idx][leg] = initPos[0] + (desiredPos[0] - initPos[0]) * idx/(PNUM) ;
//        py[idx][leg] = initPos[1] + (desiredPos[1] - initPos[1]) * idx/(PNUM);
//        pz[idx][leg] = (targetHeight) * idx/(PNUM);
    }
    px[0][leg] = initPos[0];
...
{
    double normalizedTime = (currentTime - mReferenceTime[leg]) / mTimeDuration;
